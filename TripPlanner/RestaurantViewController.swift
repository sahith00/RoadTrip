                ...
                if let JSON = response.result.value {
                    ...
                    while i < len {
                        if self.distances[i] < 0.05 {
                            YelpClient.sharedInstance.searchWithTerm("Restaurants", lat: self.endCoords.lats[i], long: self.endCoords.longs[i], limit: 10, completion: { (restaurants, error) in
                                if restaurants != nil {
                            ...
                            YelpClient.sharedInstance.searchWithTerm("Hotels", lat: midpoint.lat, long: midpoint.long, limit: 10, completion: { (restaurants, error) in
                                if restaurants != nil {
                                    for restaurant in restaurants {
                                        if let lat = restaurant.lat {
                                            if let long = restaurant.long {
